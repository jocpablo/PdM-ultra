// anexo_ultra.js — Anexo de Ultrasonido con persistencia en BD
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const TIPOS_US = ['Rodamiento', 'Fuga de Presión', 'Fuga de Vacío',
                      'Eléctrico (Corona/Arco)', 'Lubricación', 'Válvula', 'Otro'];

    // ── Fecha de hoy ──────────────────────────────────────────────
    const fechaEl = document.getElementById('au-fecha');
    if (fechaEl && !fechaEl.value) fechaEl.value = new Date().toISOString().split('T')[0];

    const analistaEl = document.getElementById('au-analista');
    ...
    async function generarCodigo() {
        ...
        if (!el) return;
        if (hidden?.value?.startsWith('AnUlt-')) { el.textContent = hidden.value; return; }
        try {
            const res = await PdM.apiFetch('/api/reportes/next-code?tecnica=anexo_ultra');
            if (res?.ok) {
            ...
            const m = ['ene','feb','mar','abr','may','jun','jul','ago','sep','oct','nov','dic'][now.getMonth()];
            const yy = String(now.getFullYear()).slice(-2);
            const code = `AnUlt-${dd}-${m}-${yy}-${String(Math.floor(Math.random()*9999)+1).padStart(4,'0')}`;
            el.textContent = code; if (hidden) hidden.value = code;
        }
    }
    ...
    // ── Tarjeta de imagen (reutiliza estilos at-img-card) ─────────
    function createImgCard(src, caption, label, onChangeSrc, onChangeCaption, onDelete) {
        ...
        card.innerHTML = `
            ...
            <div class="at-img-footer">
                <input type="text" class="at-img-caption" placeholder="Descripción..." value="${caption||''}">
        ...
        function browseFile() {
            ...
            inp.onchange = () => { if (inp.files[0]) compressImage(inp.files[0], s => { onChangeSrc(s); refreshZone(s); }); };
            inp.click();
        }
            ...
            try {
                ...
                for (const item of items) {
                    ...
                    if (t) {
                        ...
                        compressImage(new File([blob],'paste.png',{type:t}), s => { onChangeSrc(s); refreshZone(s); });
                        return;
                    }
                }
    ...
    function createPuntoSection(punto) {
        ...
        const tiposOpts = TIPOS_US.map(t =>
            `<option value="${t}" ${punto.datos?.tipo===t?'selected':''}>${t}</option>`
        ).join('');

        el.innerHTML = `
            <div class="at-punto-header">
                <div style="display:flex;align-items:center;gap:10px;flex:1;">
                    <i class="fa-solid fa-ear-listen" style="color:#93c5fd;"></i>
            ...
            <div class="at-punto-body">
                ...
                <div class="at-data-section">
                    <h5><i class="fa-solid fa-table-cells"></i> Datos de Medición</h5>
                    <table class="at-data-table">
                        <tr>
                            <td><label>Tipo Inspección:</label></td>
                            <td colspan="3">
                                <select class="at-field at-select" data-field="tipo" style="width:100%;background:transparent;border:none;border-bottom:1px solid transparent;padding:2px 4px;color:var(--text-primary);font-family:inherit;font-size:0.92em;transition:border-color .15s;">
                        ...
                        <tr>
                            <td><label>Nivel Máx. (dBµV):</label></td>
                            <td><input type="text" class="at-field" data-field="nivel_max" value="${punto.datos?.nivel_max||''}" placeholder="Ej: 42.5"></td>
                            <td><label>Nivel Base (dBµV):</label></td>
                            <td><input type="text" class="at-field" data-field="nivel_base" value="${punto.datos?.nivel_base||''}" placeholder="Ej: 28.0"></td>
                        </tr>
                        <tr>
                            <td><label>Incremento (ΔdB):</label></td>
                            <td><input type="text" class="at-field" data-field="delta_db" value="${punto.datos?.delta_db||''}" placeholder="Ej: 14.5"></td>
                            <td><label>Frecuencia Central:</label></td>
                            <td><input type="text" class="at-field" data-field="frecuencia" value="${punto.datos?.frecuencia||''}" placeholder="Ej: 40 kHz"></td>
                        </tr>
                        <tr>
                            <td><label>Sensor Utilizado:</label></td>
                            <td><input type="text" class="at-field" data-field="sensor" value="${punto.datos?.sensor||''}" placeholder="Ej: Magnético, Parabólico"></td>
                            <td><label>Ruido Ambiente (dB):</label></td>
                            <td><input type="text" class="at-field" data-field="ruido_amb" value="${punto.datos?.ruido_amb||''}" placeholder="Ej: 10"></td>
                        </tr>
                        <tr>
                            <td><label>Caract. del Sonido:</label></td>
                            <td colspan="3"><input type="text" class="at-field" data-field="carac" value="${punto.datos?.carac||''}" placeholder="Ej: Siseo continuo, Chisporroteo intermitente"></td>
                        </tr>
                        <tr>
                            <td><label>No. Imagen / Registro:</label></td>
                            <td><input type="text" class="at-field" data-field="no_imagen" value="${punto.datos?.no_imagen||''}" placeholder="Ej: IMG_001"></td>
                            <td><label>Temperatura Zona (°C):</label></td>
                            <td><input type="text" class="at-field" data-field="temp" value="${punto.datos?.temp||''}" placeholder="Ej: 45.2 °C"></td>
                        </tr>
                    </table>
                </div>

                <!-- Imágenes: espectro/onda y foto del punto -->
                <div class="at-images-section">
                    <h5><i class="fa-solid fa-chart-area"></i> Espectro / Forma de Onda e Imagen del Punto</h5>
                ...
                <div class="at-text-section">
                    <div class="at-text-col">
                        <h5><i class="fa-solid fa-circle-info"></i> Diagnóstico / Hallazgo</h5>
                        <textarea class="at-textarea at-field" data-field="estado" rows="3" placeholder="Describa el hallazgo: tipo de falla, nivel de severidad, tendencia observada...">${punto.estado||''}</textarea>
                    </div>
                    <div class="at-text-col">
        ...
        // Imágenes: espectro/onda + foto del punto
        const imgsGrid = el.querySelector(`#imgs-${punto.id}`);
        imgsGrid.appendChild(createImgCard(
            punto.imagenEspectro, punto.captionEspectro, 'Espectro / Forma de Onda',
            s => { punto.imagenEspectro = s; },
            c => { punto.captionEspectro = c; },
            () => { punto.imagenEspectro = ''; punto.captionEspectro = ''; }
        ));
        imgsGrid.appendChild(createImgCard(
            punto.imagenFoto, punto.captionFoto, 'Foto del Punto de Inspección',
            s => { punto.imagenFoto = s; },
            c => { punto.captionFoto = c; },
            () => { punto.imagenFoto = ''; punto.captionFoto = ''; }
        ));
        ...
        el.querySelectorAll('.at-field').forEach(inp => {
            const ev = inp.tagName === 'TEXTAREA' || inp.tagName === 'SELECT' ? 'input' : 'input';
        ...
        // Select also needs change event
        el.querySelectorAll('.at-select').forEach(sel => {
            sel.addEventListener('change', e => {
                if (!punto.datos) punto.datos = {};
                punto.datos[e.target.dataset.field] = e.target.value;
            });
            sel.addEventListener('focus', () => { sel.style.borderBottomColor = 'var(--primary)'; });
        ...
        el.querySelector('.at-del-punto-btn').addEventListener('click', () => {
            if (!confirm('¿Eliminar este punto de ultrasonido?')) return;
    ...
    document.getElementById('btn-add-punto')?.addEventListener('click', () => {
        ...
        const p = { id: Date.now(), nombre: `Punto Ultrasonido ${num}`,
                    datos: {}, imagenEspectro:'', captionEspectro:'',
                    imagenFoto:'', captionFoto:'', estado:'', acciones:'', sev:'' };
        puntos.push(p);
        const container = document.getElementById('puntos-container');
        container.appendChild(createPuntoSection(p));
        updateEmptyMsg();
        setTimeout(() => container.lastElementChild?.scrollIntoView({ behavior:'smooth', block:'start' }), 100);
    });

    // ── Recolectar / Restaurar datos ──────────────────────────────
    function collectData() {
        const info = {};
        ['au-equipo','au-ubicacion','au-fecha','au-analista','au-equipo-us','au-frecuencia'].forEach(id => {
            const el = document.getElementById(id); if (el) info[id] = el.value;
        });
        const raEl = document.getElementById('au-reporte-asociado'); if(raEl) info['au-reporte-asociado'] = raEl.value;
        return { codigo: document.getElementById('current_report_id')?.value||'',
                 info, puntos, firma: document.querySelector('.cargo-analista')?.value||'' };
    }
    ...
    function restoreData(datos) {
        ...
        if (datos.info) Object.entries(datos.info).forEach(([id,val]) => { const e=document.getElementById(id); if(e) e.value=val; });
        if (datos.firma) { const e=document.querySelector('.cargo-analista'); if(e) e.value=datos.firma; }
    ...
    async function guardarReporte() {
        ...
        try {
            ...
            if (!dbId) {
                res = await PdM.apiFetch('/api/reportes', { method:'POST', body:JSON.stringify({ tecnica:'anexo_ultra', titulo:codigo, datos, codigo_reporte:codigo }) });
                if (res?.ok) {
                    const d = await res.json();
                    const h = document.getElementById('db_report_id');
                    if (h) h.value = d.id;
    ...
    async function mostrarLista() {
        try {
            const res = await PdM.apiFetch('/api/reportes?tecnica=anexo_ultra');
            if (!res?.ok) return toast('Error al obtener lista.','error');
            const lista = await res.json();
            if (!lista.length) return toast('No hay anexos de ultrasonido guardados.','info');
            ...
            modal.innerHTML=`<div class="modal-content config-modal-box" style="max-width:600px;width:95%;">
                <div class="modal-header"><h3><i class="fa-solid fa-folder-open"></i> Anexos Guardados — Ultrasonido</h3>
                <span style="cursor:pointer;font-size:1.4em;color:var(--text-muted);" id="close-rl">&times;</span></div>
                <div id="lista-rl" style="max-height:420px;overflow-y:auto;"></div></div>`;
            ...
            document.querySelectorAll('.btn-abrir').forEach(btn => {
                btn.onclick = async () => {
                    ...
                    if(!r2?.ok) return toast('Error.','error');
    ...
    async function buscarPorCodigo() {
        ...
        if (!codigo) { toast('Ingresa un código. Ej: AnUlt-19-mar-26-0001','info'); return; }
        try {
    ...
    document.getElementById('btn-nuevo-reporte')?.addEventListener('click', () => {
        ...
        updateEmptyMsg();
        ['au-equipo','au-ubicacion','au-analista','au-equipo-us','au-frecuencia'].forEach(id=>{const e=document.getElementById(id);if(e)e.value='';});
        document.getElementById('au-fecha').value = new Date().toISOString().split('T')[0];
        document.querySelector('.cargo-analista').value='';
        // ── Reporte Asociado — lookup en vivo ───────────────────────
    const raInput = document.getElementById('au-reporte-asociado');
    const raLink  = document.getElementById('au-link-reporte');
    if (raInput && raLink) {
    ...
    window.addEventListener('beforeprint', () => {
        const c = document.getElementById('visual-report-id')?.textContent?.trim();
        if (c && !c.includes('Generando')) document.title = 'Anexo Ultrasonido — ' + c;
    });
